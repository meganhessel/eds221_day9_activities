...
mack_creek_lengths<- mack_creek_vertebrates %>% 
  ...
  filter(section == "CC") %>% 
  mutate(across(where(is.character), ~tolower(.))) %>% #PAST ERROR: tolower needs a ~ and a argument
  drop_na(species) %>% 
  group_by(species) %>% 
...
```{r}
# create a subset 
reprex1_sub <- mack_creek_vertebrates %>% 
  clean_names() %>% 
  select(year:sampledate) %>% 
  drop_na(species) %>% 
  group_by(species) %>% 
...
# datapasta 
datapasta::df_paste(reprex1_sub)
...
# subset of dataframe
reprex1_sub <- data.frame(
  stringsAsFactors = FALSE,
           species = c("DITE", "ONCL", "RHOL"),
    mean_length_cm = c(57.0159162481919, 83.531329547679, 35.9333333333333),
      sd_length_cm = c(20.8706475988055, 35.376212710866, 5.20256347070045)
   )

# CANT FIND THE ERROR HERE 
reprex1_sub %>% 
...
mack_creek_vertebrates %>% 
  ...
  ggplot(aes(x = length1, 
             y = weight)) +
         geom_point(color = "purple", #ERROR: these variables should not be in aes() 
                    shape = 12) +
         theme_minimal() +
